...
<%--
	@modelObject - StatsNavigationDTO statsNavigation - object containing all navigation in stats component(submenu, column sort, pagination)
	
statistics-conferences, statistics-divisions:
	@modelObject - TeamStandingsDTO teamStandings - team standings data for both conferences and divisions

statistics-skaters:
	@modelObject - StatsNavigationDTO statsNavigation - object containing all navigation in stats component(submenu, column sort, pagination)
	@modelObject - List<SkaterStatsDTO> skaterStats - list with all skater's data filtered by statsNavigation
--%>
...
<form:form modelAttribute="statsNavigation" method="GET">
	...
	<c:choose>
		<c:when
			...
			<jsp:include page="/WEB-INF/views/sub_components/statistics-conferences.jsp"></jsp:include>
		</c:when>
		<c:when
			test="${statsNavigation.dataScope == 'Teams' && statsNavigation.standingsScope == 'Division'}">
			<jsp:include page="/WEB-INF/views/sub_components/statistics-divisions.jsp"></jsp:include>
		</c:when>
		<c:when test="${statsNavigation.dataScope == 'Players'}">
			<div id="tblPopupNav">
				<input type="button" class="tblPopupResize" title="Maximize/Minimize Table" value="+">
			</div>
			<jsp:include page="/WEB-INF/views/sub_components/statistics-skaters.jsp"></jsp:include>
