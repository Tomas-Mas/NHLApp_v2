...
from (
    select data.g_id, data.p_id, data.firstName, data.lastName, data.position, data.season, data.gameType, data.team, data.timeOnIce, data.plusMinus,
            ...
            case when data.name = 'Goal' and data.role = 'Scorer' and (data.periodType = 'REGULAR' or data.periodType = 'OVERTIME') then 1 else 0 end as Goals,
            case when data.name = 'Goal' and data.role = 'Assist' then 1 else 0 end as Assists,
            case when data.name = 'Goal' and (data.role = 'Scorer' OR data.role = 'Assist') and (data.periodType = 'REGULAR' or data.periodType = 'OVERTIME') then 1 else 0 end as Points,
            ...
            case when data.name = 'Goal' and data.periodType = 'OVERTIME' and data.role = 'Scorer' then 1 else 0 end as OTGoals,
            case when data.periodType = 'SHOOTOUT' and data.role != 'Goalie' then 1 else 0 end as ShootoutTaken,
            case when data.name = 'Goal' and data.periodType = 'SHOOTOUT' and data.role = 'Scorer' then 1 else 0 end as ShootoutGoals,
            case when data.name = 'Penalty' and data.role = 'PenaltyOn' then data.penaltyMinutes else 0 end as PIM,
            case when data.name = 'Shot' and data.role = 'Shooter' and data.periodType != 'SHOOTOUT' then 1
                when data.name = 'Goal' and data.role = 'Scorer' and (data.periodType = 'REGULAR' or data.periodType = 'OVERTIME') then 1 else 0 end as Shots,
    ...
    from (
        select g.g_id, g.season, g.gameType,
                ...
                NVL(ss.plusMinus, 0) as plusMinus, NVL(SUBSTR(ss.timeOnIce, 0, INSTR(ss.timeOnIce, ':') - 1) * 60, 0) + NVL(SUBSTR(ss.timeOnIce, INSTR(ss.timeOnIce, ':') + 1), 0) as timeOnIce
        from games g
            ...
            left join SkaterStats ss on ss.ss_id = r.ss_id
